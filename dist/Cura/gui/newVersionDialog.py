...
class newVersionDialog(wx.Dialog):
	def __init__(self):
		...
		title = wx.StaticText(p, -1, 'Fracktory - ' + version.getVersion())
		title.SetFont(wx.Font(18, wx.SWISS, wx.NORMAL, wx.BOLD))
		s.Add(title, flag=wx.ALIGN_CENTRE|wx.EXPAND|wx.BOTTOM, border=5)
		s.Add(wx.StaticText(p, -1, 'Welcome to the new version of Fracktory.'))
		s.Add(wx.StaticText(p, -1, '(This dialog is only shown once)'))
		s.Add(wx.StaticLine(p), flag=wx.EXPAND|wx.TOP|wx.BOTTOM, border=10)
		s.Add(wx.StaticText(p, -1, 'New in this version:'))
		s.Add(wx.StaticText(p, -1, '* Added New Machines'))
		s.Add(wx.StaticText(p, -1, '* Saves a preview image along with the sliced gcode'))
		s.Add(wx.StaticText(p, -1, '* Changed Quality Settings slightly'))
		s.Add(wx.StaticText(p, -1, '* Calibrated printing profiles for multiple materials'))
		s.Add(wx.StaticText(p, -1, '* Default firmware flash directly from machine menu'))
		s.Add(wx.StaticText(p, -1, '* Wireless printing capabilities with printers on the same network**'))
