...
from Cura.gui import tweakerWindow
from Cura.WirelessPrinting import uploadWindow as WirelessUpload
...
class SceneView(openglGui.glGuiPanel):
	def __init__(self, parent):
		...
		# self.iSliceButton         = openglGui.glButton(self, 61, _("Auto-Orient beta*"), (4,0), self.OniSliceButton)
		# self.orderButton         = openglGui.glButton(self, 59, _("Order Filament"), (4,0), self.OnOrderButton)
		self.orderButton = openglGui.glButton(self, 26, _("Shop"), (4,0), lambda value : webbrowser.open('http://www.fracktal.in/shop'))
	...
	def OnPrintButton(self, button):
		if button == 1:
			...
			elif connectionGroup is None:
				self.notification.message("Printer Not connected")
	...
	def OnPrintButton2(self, button):
 	...
 	def OniSliceButton(self, button):
		window = tweakerWindow.tweakerWindow(self)
		window.Show()

 	def OniSliceOptimize(self):
	    	'''
	    	Funtion that calculates the optimal orientation fot the loaded STL model on screen
	    	1. Get location of the current STL file
	    	2. Pass the location to the Tweaker algorithm
	    	3. Delete objects on screen and Reload the tweaked stl
	    	4. Automated support enable
	    	'''
		# Move this to profile/settings later
		tweakerLocation = os.path.normpath(os.path.join(os.path.split(__file__)[0], "../iSlicer/Tweaker.exe"))
		print tweakerLocation
		def launch_tweaker():
			# self.OnDeleteAll(None)
			try:
				bi = wx.BusyInfo("Calculating Optimal Orientation, Please Wait", self)
				lastFile = os.path.normpath(profile.getPreference('lastFile'))
				scaled_filename = [os.path.normpath(os.path.splitext(lastFile)[0]+"_scaled.stl")]
				meshLoader.saveMeshes(scaled_filename[0], self._scene.objects())
				self.OnDeleteAll(None)
				profile.putPreference('lastFile', scaled_filename[0])
				subprocess.check_call([tweakerLocation, "-i", scaled_filename, "-b"],shell=True,stderr=subprocess.STDOUT)
				os.remove(scaled_filename[0])
			except Exception as e:
				print e

			try:
				tweaked_filename = [os.path.normpath(os.path.splitext(os.path.normpath(profile.getPreference('lastFile')))[0]+"_tweaked.stl")]
				print tweaked_filename
				print "\n"
				with open(tweaked_filename[0]) as fin:
					lastline = deque(fin,1)
					print lastline
					mainWindow = self.GetParent().GetParent().GetParent()
					if lastline[-1].find('yes')!=-1:
						# enable support here
						print "enabling support \n"
						mainWindow.simpleSettingsPanel.printSupport.SetValue(True)
						profile.putProfileSetting('support', 'Touching buildplate')
					else:
						# Disable support here
						mainWindow.simpleSettingsPanel.printSupport.SetValue(False)
						profile.putProfileSetting('support', 'None')
						# profile.putPreference('lastFile', tweaked_filename[0])
					self.loadFiles(tweaked_filename)
			except Exception as e:
				self.notification.message("Failed to calculate optimal orientation")
				# print e
			bi.Destroy()
			self.notification.message("Auto-Orientation Complete...")

		threading.Thread(target=launch_tweaker).start()


	def OnDirectiSlice(self):
		'''
		Use Python module directly instead of binary
		'''


 	def OnOrderButton(self, button):
		pass

 	def OnWirelessPrintButton(self, button):
		'''
		Function that sends the current gcode to the printer using an API Call
		Conditions:
		1. Enable wireless button if gcode is sliced and ready
		2. Open window on click which enables us to select the printer from dropdown
		3. Upload to selected printer and show status loading bar
		4. Start print and open monitor window
		'''
		printer_list = []
		for n in xrange(0, profile.getMachineCount()):
			# printer_list.append(profile.getMachineSetting('machine_name', n).title())
			# if n == int(profile.getPreferenceFloat('active_machine')):
			# 	i.Check(True)
			printer_list.append([])
			m_name = profile.getMachineSetting('machine_name', n)
			m_ip = profile.getMachineSetting('wireless_ip', n)
			m_api = profile.getMachineSetting('wireless_api', n)
			printer_list[n].append(m_name)
			printer_list[n].append(m_ip)
			printer_list[n].append(m_api)
		...
		threading.Thread(target=self._saveGCode,args=(os.path.join(profile.getBasePath(),'WirelessTemp\\' + filename),)).start()

		f1 = WirelessUpload.uploadWindow(self,printer_list,os.path.join(profile.getBasePath(),'WirelessTemp/' + filename))
		f1.Show()
	...
	def showSaveGCode(self):
		...
		filename = self._scene._objectList[0].getName() + profile.getGCodeExtension()
		dlg.SetFilename(filename.replace(" ", ""))
		...
		filename = os.path.normpath(os.path.splitext(filename)[0]+ profile.getGCodeExtension())
		print filename
		dlg.Destroy()
	...
	def _saveGCode(self, targetFilename, ejectDrive = False):
		...
		try:
			png_filename = [os.path.normpath(os.path.splitext(targetFilename)[0]+".png")]
			openscadLocation = os.path.normpath(os.path.join(os.path.split(__file__)[0], "../openscad/openscad.exe"))
			openscadTemplateLocation = os.path.normpath(os.path.join(os.path.split(__file__)[0], "../openscad/stlpreview.scad"))
			# get object name for temp stl
			filename_temp = self._scene._objectList[0].getName() + '.stl'
			filename_temp_scad = self._scene._objectList[0].getName() + '.scad'
			filepath_temp_scad = os.path.normpath(os.path.join(profile.getBasePath(),'WirelessTemp', filename_temp_scad))
			# temp STL save location
			temp_filename = [os.path.normpath(os.path.join(profile.getBasePath(),'WirelessTemp', filename_temp))]
			# save STL
			# sentence.replace(" ", "")
			meshLoader.saveMeshes(temp_filename[0], self._scene.objects())
			# CALL openscad
			# write temp scadfile and call it
			tmpScadFileContents = 'model="{0}";\nimport(file=model);'
			tmpScadFile = open(filepath_temp_scad, 'w')
			tmpScadFile.write(tmpScadFileContents.format(os.path.normpath(temp_filename[0]).replace('\\','/')))
			tmpScadFile.flush()

			subprocess.check_call(r'{} "{}" --colorscheme Starnight --autocenter -o "{}"'.format(openscadLocation, filepath_temp_scad, png_filename[0]) ,shell=False,stderr=subprocess.STDOUT)

			# print '{} "{}" --colorscheme Starnight --autocenter -o "{}"'.format(openscadLocation, filepath_temp_scad, png_filename[0])
			# remove temp file
			os.remove(temp_filename[0])
			os.remove(filepath_temp_scad)
		except Exception as e:
			self.notification.message("Failed to save STL preview image")
			print e
	...
	def _drawMachine(self):
		...
		if machine.startswith('juliaplus'):
			if machine not in self._platformMesh:
				meshes = meshLoader.loadMeshes(resources.getPathForMesh(machine + '_platform.stl'))
				if len(meshes) > 0:
					self._platformMesh[machine] = meshes[0]
					...
					self._platformMesh[machine] = None
				if machine == 'juliaplus':
					self._platformMesh[machine]._drawOffset = numpy.array([0,15.2,22.6], numpy.float32)
