...
setting('simpleModeSupportType', '1', str, 'preference', 'hidden')
...
setting('auto_slice', 'True', bool, 'preference', 'hidden').setLabel(_("Slice automatically"), _("Begin slicing automatically"))
...
setting('city', '', str, 'machine', 'hidden').setLabel(_("City"), _("City"))
setting('email', '', str, 'machine', 'hidden').setLabel(_("Email"), _("Email"))
setting('sn_number', '', str, 'machine', 'hidden').setLabel(_("Serial Number"), _("Serial Number for the printer"))
...
def getBasePath():
	...
	if platform.system() == "Windows":
		basePath = os.path.normpath(os.path.expanduser('~/.fracktory/%s' % version.getVersion(False)))
	...
	if not os.path.isdir(basePath):
		try:
			...
			os.makedirs(basePath+"/WirelessTemp/")
		except:
	...
	try:
		...
		for subPath in os.listdir(basePath):
			...
			if os.path.isdir(path) and os.path.isfile(os.path.join(path, 'preferences.ini')) and path != getBasePath():
				paths.append(path)
			path = os.path.join(basePath, subPath, 'fracktory')
			if os.path.isdir(path) and os.path.isfile(os.path.join(path, 'preferences.ini')) and path != getBasePath():
				paths.append(path)
		paths.sort()
